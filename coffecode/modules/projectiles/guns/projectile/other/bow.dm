...
/obj/item/gun/projectile/bow
	...
	arrow_x_offset_per_tension = -1
	arrow_y_offset_per_tension = 1

/obj/item/gun/projectile/bow/update_draw()
...
/obj/item/gun/projectile/bow/release_charge(atom/target, mob/living/user)
	...
	// If the bow is released extremely early, overcharge_level can still be 0.
	// projectile.multiply_projectile_damage(0) effectively nulls damage, causing "almost no damage" behavior.
	// Clamp to 1 to guarantee at least baseline damage scaling.
	var/damage_mult = max(1, overcharge_level) * 3

	Fire(target, user,
		extra_proj_damagemult = damage_mult,
		extra_proj_penmult = damage_mult,
		extra_proj_stepdelaymult = (overcharge_level > 5 ? 1 : 0.4),
		multiply_projectile_agony = damage_mult)
	overcharge_level = 0
	update_icon()
